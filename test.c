...
struct sched_attr {
	...
	/* New sjnprio fields */
	uint64_t sjnprio_time;
	uint64_t sjnprio_prio;
	uint64_t sjnprio_id;
};
...
void run_child(int n) {
	...
	attr.sjnprio_id = child_id;
	// children have deadlines ranges from 2s down to 400ms
	// later children have sooner deadlines, so should schedule first
	attr.sjnprio_time = 2000 * millis -  child_id * 100 * millis; //later children have sooner deadlines
	
	if(child_id > 7){
		attr.sjnprio_time = 300 * millis;
		attr.sjnprio_prio = child_id;
	}
	else {
		attr.sjnprio_prio = 1;
	}
